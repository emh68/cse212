...
public static class MapsTester
{
    ...
    {
        // Ensure that strings are lowercase letters and spaces are removed
        string NormalizeString(string str)
        {
            return new string(str.ToLower().Where(char.IsLetter).ToArray());
        }
        ...
        // If character count is not the same return false for IsAnagram
        if (charCount1.Count != charCount2.Count)
        {
            return false;
        }

        // Compare the key value pairs in the dictionaries to see if they are the same
    ...
    private static void EarthquakeDailySummary()
    {
        ...
        {
            Console.WriteLine("No features found or deserialization failed.");
        }
    }
}